...
class CallModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    fun endCall(promise: Promise) {
        try {
            ...
            // This requires ANSWER_PHONE_CALLS permission on Android 9+
            // For now, we'll just return success as ending calls programmatically
            // has restrictions on modern Android versions
            promise.resolve(true)
        } catch (e: Exception) {
